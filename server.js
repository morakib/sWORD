...
class GameRoom {
  constructor(roomId) {
    ...
    this.timer = null;
    this.timeLeft = 10;
    this.isTransitioning = false; // Added a flag to track transition state
  }
  ...
  nextTurn() {
    ...
    // Wait for 3 seconds before actually changing the turn
    setTimeout(() => {
  ...
  async checkWords(x, y) {
    ...
    const verticalLetters = Array.from(
      { length: bottom - top + 1 },
      (_, i) => this.grid[top + i][y]
    );
...
function updateAllClients(roomId) {
  ...
  io.to(roomId).emit("game-state", {
    ...
    isTransitioning: room.isTransitioning, // Send transitioning state
    nextPlayer:
      room.players[(room.currentPlayerIndex + 1) % room.players.length], // Send next player
  });
}
...
io.on("connection", (socket) => {
  ...
  socket.on("letter-placed", async ({ position, letter }) => {
    ...
    // Prevent placing letters during transition
